...
// New empty board with one starter group. Appended to the end of the list.
export async function createBoard(sb, { name }) {
  ...
  const { data: group } = await sb.from('groups')
    .insert({ board_id: board.id, title: 'Deals', color: '#0073ea', position: 0 })
    .select().single();
  return { ...board, groups: group ? [group] : [] };
}
